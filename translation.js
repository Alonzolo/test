...
//							out.push( result.sentences[i].trans.replace(/ ([,.]) /g, "$1 ").replace(/ ([,.])$/g, "$1") );		// latest time google return commas in spaces :/
			...
			{
				outputStrings[idx] = WORDS.msgErrorCode+": " + result.error.statusCode + " - " + result.error.parsedDetails.join("");
			}
...
function TranslateByPromt(from_lang, to_lang, text)
{
	...
	{
		var str = post_vars.eventName + "#" + post_vars.dirCode + "#" + post_vars.topic + "#" + local_text_hash(post_vars.text).toString();
